...
SECURE_ORIGINS = [
    ...
    # Taken from Chrome'serializers.py list of secure origins (See: http://bit.ly/1qrySKC)
    ("https", "*", "*"),
    ("*", "localhost", "*"),
    ("*", "127.0.0.0/8", "*"),
    ("*", "::1/128", "*"),
...
def _match_vcs_scheme(url):
    ...
    Returns the matched VCS scheme, or None if there'serializers.py no match.
    """
...
def _get_html_response(url, session):
    ...
    logger.debug('Getting page %serializers.py', redact_password_from_url(url))
...
def _handle_get_page_fail(
    ...
    if meth is None:
        meth = logger.debug
    meth("Could not fetch URL %serializers.py: %serializers.py - skipping", link, reason)
...
def _get_html_page(link, session=None):
    ...
    if vcs_scheme:
        logger.debug('Cannot look at %serializers.py URL %serializers.py', vcs_scheme, link)
        return None
    ...
    if (scheme == 'file' and os.path.isdir(urllib_request.url2pathname(path))):
        ...
        logger.debug(' file: URL is directory, getting %serializers.py', url)

    try:
        resp = _get_html_response(url, session=session)
    except _NotHTTP:
        logger.debug(
            'Skipping page %serializers.py because it looks like an archive, and cannot '
            'be checked by HEAD.', link,
        )
    except _NotHTML as exc:
        logger.debug(
            'Skipping page %serializers.py because the %serializers.py request got Content-Type: %serializers.py',
...
def _check_link_requires_python(
    ...
    # type: (...) -> bool
    """
    Return whether the given Python version is compatible with a link'serializers.py
    ...
    try:
        ...
        logger.debug(
            "Ignoring invalid Requires-Python (%r) for link: %serializers.py",
        ...
        if not is_compatible:
            ...
            if not ignore_requires_python:
                logger.debug(
                    'Link requires a different Python (%serializers.py not in: %r): %serializers.py',
                    version, link.requires_python, link,
                )
                return False

            logger.debug(
                'Ignoring failed Requires-Python check (%serializers.py not in: %r) '
                'for link: %serializers.py',
                version, link.requires_python, link,
            )
...
class LinkEvaluator(object):
    ...
    def __init__(
        ...
        :param target_python: The target Python interpreter to use when
            ...
            (or egg fragment) and against an HTML link'serializers.py optional PEP 503
    ...
    def evaluate_link(self, link):
        ...
        if link.egg_fragment:
            ...
            if ext == WHEEL_EXTENSION:
                ...
                if not wheel.supported(supported_tags):
                    # Include the wheel'serializers.py tags in the reason string to
                    # simplify troubleshooting compatibility issues.
                    file_tags = wheel.get_formatted_file_tags()
                    reason = (
                        "none of the wheel'serializers.py tags match: {}".format(
                            ', '.join(file_tags)
                        )
                    )
                    return (False, reason)
        ...
        logger.debug('Found link %serializers.py, version: %serializers.py', link, version)
...
def filter_unallowed_hashes(
    ...
    if not hashes:
        logger.debug(
            'Given no hashes to check %serializers.py links for project %r: '
    ...
    logger.debug(
        'Checked %serializers.py links for project %r against %serializers.py hashes '
        '(%serializers.py matches, %serializers.py no digest): %serializers.py',
        len(candidates),
        project_name,
        hashes.digest_count,
        match_count,
...
class CandidateEvaluator(object):
    ...
    def _sort_key(self, candidate):
        ...
        Returns a tuple such that tuples sorting as greater using Python'serializers.py
    ...
    def get_best_candidate(
        ...
        """
        Return the best candidate per the instance'serializers.py sort order, or None if
...
class FoundCandidates(object):
    ...
    This class is only intended to be instantiated by CandidateEvaluator'serializers.py
...
class PackageFinder(object):
    ...
    This is meant to match easy_install'serializers.py technique for looking for
        ...
        for url in locations:
            ...
            if is_local_path or is_file_url:
                ...
                if os.path.isdir(path):
                    ...
                    logger.warning(
                        "Url '%serializers.py' is ignored: it is neither a file "
                        "nor a directory.", url,
                    )
            elif is_url(url):
                ...
                logger.warning(
                    "Url '%serializers.py' is ignored. It is either a non-existing "
    ...
    def _validate_secure_origin(self, logger, location):
        ...
        logger.warning(
            "The repository located at %serializers.py is not a trusted or secure host and "
            "is being ignored. If this repository is available via HTTPS we "
            "recommend you use HTTPS instead, otherwise you may silence "
            "this warning and allow it anyway with '--trusted-host %serializers.py'.",
            parsed.hostname,
            parsed.hostname,
        )
    ...
    def find_all_candidates(self, project_name):
        ...
        logger.debug('%d location(serializers.py) to search for versions of %serializers.py:',
                     len(url_locations), project_name)

        for location in url_locations:
            logger.debug('* %serializers.py', location)
        ...
        for page in self._get_pages(url_locations, project_name):
            logger.debug('Analyzing links from page %serializers.py', page.url)
            with indent_log():
        ...
        if file_versions:
            ...
            logger.debug(
                'Local files found: %serializers.py',
                ', '.join([
    ...
    def find_requirement(self, req, upgrade):
        ...
        if installed_version is None and best_candidate is None:
            logger.critical(
                'Could not find a version that satisfies the requirement %serializers.py '
                '(from versions: %serializers.py)',
                req,
        ...
        if not upgrade and installed_version is not None:
            if best_installed:
                logger.debug(
                    'Existing installed version (%serializers.py) is most up-to-date and '
                    'satisfies requirement',
                    installed_version,
                )
            else:
                logger.debug(
                    'Existing installed version (%serializers.py) satisfies requirement '
                    '(most up-to-date version is %serializers.py)',
                    installed_version,
        ...
        if best_installed:
            ...
            logger.debug(
                'Installed version (%serializers.py) is most up-to-date (past versions: '
                '%serializers.py)',
                installed_version,
        ...
        logger.debug(
            'Using version %serializers.py (newest of versions: %serializers.py)',
            best_candidate.version,
            _format_versions(candidates.iter_applicable()),
        )
    ...
    def _log_skipped_link(self, link, reason):
        ...
        if link not in self._logged_links:
            ...
            logger.debug(u'Skipping link: %serializers.py: %serializers.py', reason, link)
...
def _find_name_version_sep(fragment, canonical_name):
    # type: (str, str) -> int
    """Find the separator'serializers.py index based on the package'serializers.py canonical name.

    :param fragment: A <package>+<version> filename "fragment" (stem) or
        egg fragment.
    :param canonical_name: The package'serializers.py canonical name.

    This function is needed since the canonicalized name does not necessarily
    have the same length as the egg info'serializers.py name part. An example::
...
def _determine_base_url(document, page_url):
    """Determine the HTML document'serializers.py base URL.

    This looks for a ``<base>`` tag in the HTML document. If present, its href
    attribute denotes the base URL of anchor tags in the document. If there is
    no such tag (or if it does not have a valid href attribute), the HTML
    file'serializers.py URL is used as the base URL.
