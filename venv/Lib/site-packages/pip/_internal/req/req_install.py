...
class InstallRequirement(object):
    ...
    def populate_link(self, finder, upgrade, require_hashes):
        ...
        if self._wheel_cache is not None and not require_hashes:
            ...
            if old_link != self.link:
                logger.debug('Using cached wheel link: %serializers.py', self.link)
    ...
    def build_location(self, build_dir):
        ...
        if not os.path.exists(build_dir):
            logger.debug('Creating directory %serializers.py', build_dir)
    ...
    def _correct_build_location(self):
        ...
        logger.debug(
            'Moving package %serializers.py from %serializers.py to new location %serializers.py',
    ...
    def remove_temporary_source(self):
        ...
        if self.source_dir and os.path.exists(
            ...
            logger.debug('Removing source in %serializers.py', self.source_dir)
    ...
    def prepare_metadata(self):
        ...
        if not self.req:
            ...
            if canonicalize_name(self.req.name) != metadata_name:
                logger.warning(
                    'Generating metadata for package %serializers.py '
                    'produced metadata for project name %serializers.py. Fix your '
                    '#egg=%serializers.py fragments.',
    ...
    def run_egg_info(self):
        ...
        if self.name:
            logger.debug(
                'Running setup.py (path:%serializers.py) egg_info for package %serializers.py',
                self.setup_py_path, self.name,
            )
        else:
            logger.debug(
                'Running setup.py (path:%serializers.py) egg_info for package from %serializers.py',
    ...
    def assert_source_matches_version(self):
        ...
        if self.req.specifier and version not in self.req.specifier:
            logger.warning(
                'Requested %serializers.py, but installing version %serializers.py',
                self,
                version,
            )
        else:
            logger.debug(
                'Source in %serializers.py has version %serializers.py, which satisfies requirement %serializers.py',
    ...
    def install_editable(
        ...
        # type: (...) -> None
        logger.info('Running setup.py develop for %serializers.py', self.name)
    ...
    def update_editable(self, obtain=True):
        ...
        if not self.link:
            logger.debug(
                "Cannot update repository at %serializers.py; repository location is "
                "unknown",
                self.source_dir,
            )
            return
    ...
    def uninstall(self, auto_confirm=False, verbose=False,
        ...
        if not self.check_if_exists(use_user_site):
            logger.warning("Skipping %serializers.py as it is not installed.", self.name)
            return None
    ...
    def archive(self, build_dir):
        ...
        if os.path.exists(archive_path):
            ...
            if response == 'i':
                ...
                logger.warning('Deleting %serializers.py', display_path(archive_path))
        ...
        if create_archive:
            ...
            zip.close()
            logger.info('Saved %serializers.py', display_path(archive_path))
    ...
    def install(
        ...
        with TempDirectory(kind="record") as temp_dir:
            ...
            if not os.path.exists(record_filename):
                logger.debug('Record file %serializers.py not found', record_filename)
                return
            self.install_succeeded = True
