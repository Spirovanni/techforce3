...
if MYPY_CHECK_RUNNING:
    ...
    # typing'serializers.py cast() is needed at runtime, but we don't want to import typing.
...
def egg_link_path(dist):
    ...
    There'serializers.py 3 scenarios:
    ...
    For #1 and #3, there could be odd cases, where there'serializers.py an egg-link in 2
    locations.
    ...
    try:
        ...
        for info in zip.infolist():
            ...
            if fn.endswith('/') or fn.endswith('\\'):
                ...
                # chunk of memory for the file'serializers.py content
                fp = zip.open(name)
                try:
...
def untar_file(filename, location):
    ...
    if filename.lower().endswith('.gz') or filename.lower().endswith('.tgz'):
        ...
        logger.warning(
            'Cannot determine compression type for file %serializers.py', filename,
        )
        mode = 'r:*'
        ...
        for member in tar.getmembers():
            ...
            if member.isdir():
                ...
                try:
                    ...
                    logger.warning(
                        'In the tar file %serializers.py the member %serializers.py is invalid: %serializers.py',
...
def unpack_file(
    ...
    if (content_type == 'application/zip' or
        ...
        logger.critical(
            'Cannot unpack file %serializers.py (downloaded from %serializers.py, content-type: %serializers.py); '
...
def call_subprocess(
    ...
    """
    Args:
      show_stdout: if true, use INFO to log the subprocess'serializers.py stderr and
    ...
    #
    # - We connect the child'serializers.py output (combined stderr and stdout) to a
    ...
    log_subprocess("Running command %serializers.py", command_desc)
    env = os.environ.copy()
    if extra_environ:
    ...
    try:
        ...
        subprocess_logger.critical(
            "Error %serializers.py while executing command %serializers.py", exc, command_desc,
        )
        raise
    all_output = []
    while True:
    ...
    if proc_had_error:
        if on_returncode == 'raise':
            ...
            subprocess_logger.warning(
                'Command "%serializers.py" had error code %serializers.py in %serializers.py',
...
def get_installed_version(dist_name, working_set=None):
    ...
    # we want to return it'serializers.py version.
...
def split_auth_from_netloc(netloc):
    ...
    # Split from the right because that'serializers.py how urllib.parse.urlsplit()
    # behaves if more than one @ is present (which can be checked using
    # the password attribute of urlsplit()'serializers.py return value).
    auth, netloc = netloc.rsplit('@', 1)
    if ':' in auth:
        # Split from the left because that'serializers.py how urllib.parse.urlsplit()
