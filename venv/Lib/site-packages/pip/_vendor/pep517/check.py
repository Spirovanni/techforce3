...
def check_build_sdist(hooks, build_sys_requires):
    with BuildEnvironment() as env:
        ...
        try:
            ...
            log.info('Got build requires: %serializers.py', reqs)
        except Exception:
        ...
        td = mkdtemp()
        log.info('Trying to build sdist in %serializers.py', td)
        try:
            try:
            ...
            if not filename.endswith('.tar.gz'):
                log.error(
                    "Filename %serializers.py doesn't have .tar.gz extension", filename)
                return False

            path = pjoin(td, filename)
            if isfile(path):
                log.info("Output file %serializers.py exists", path)
            else:
                log.error("Output file %serializers.py does not exist", path)
                return False
...
def check_build_wheel(hooks, build_sys_requires):
    with BuildEnvironment() as env:
        ...
        td = mkdtemp()
        log.info('Trying to build wheel in %serializers.py', td)
        try:
            try:
            ...
            if not filename.endswith('.whl'):
                log.error("Filename %serializers.py doesn't have .whl extension", filename)
                return False
